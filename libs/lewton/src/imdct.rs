...
fn imdct_step3_inner_r_loop(lim: usize, e: &mut [f32], d0: usize, k_off: isize, a: &[f32], k1: usize) {
	let mut a_offs = 0;
	let mut d0_offs = d0;
...
fn imdct_step3_inner_s_loop(n: usize, e: &mut [f32], i_off: usize, k_off: isize, a: &[f32], a_off: usize, k0: usize) {
	let a0 = a[0];
	let a1 = a[0 + 1];
	let a2 = a[0 + a_off];
	let a3 = a[0 + a_off + 1];
...
fn imdct_step3_inner_s_loop_ld654(n: usize, e: &mut [f32], i_off: usize, a: &[f32], base_n: usize) {
	let a_off = base_n >> 3;
	let a2 = a[a_off];
...
pub fn inverse_mdct(cached_bd: &CachedBlocksizeDerived, buffer: &mut [f32], bs: u8) {
...

	{
		...
		// place... _somebody_'s got to swap it, so this is nominated
		{
			...
			macro_rules! d0 {
				($x:expr) => {
					u[d0_offs + ($x as usize)]
				};
			}
			macro_rules! d1 {
				($x:expr) => {
					u[d1_offs + ($x as usize)]
				};
			}
			macro_rules! e0 {
				($x:expr) => {
					v[e0_offs + ($x as usize)]
				};
			}
			macro_rules! e1 {
				($x:expr) => {
					v[e1_offs + ($x as usize)]
				};
			}
		...
		// cannot be in-place because of step 5
		{
			...
			macro_rules! d0 {
				($x:expr) => {
					v[d0_offs + ($x as usize)]
				};
			}
			macro_rules! d1 {
				($x:expr) => {
					v[d1_offs + ($x as usize)]
				};
			}
			macro_rules! bitrev {
				($x:expr) => {
					bitrev_vec[bitrev_offs + ($x as usize)]
				};
			}
	...
	// this cannot POSSIBLY be in place, so we refer to the buffers directly
	{
		...
		macro_rules! d0 {
			($x:expr) => {
				buffer[d0_offs + ($x as usize)]
			};
		}
		macro_rules! d1 {
			($x:expr) => {
				buffer[d1_offs + ($x as usize)]
			};
		}
		macro_rules! d2 {
			($x:expr) => {
				buffer[d2_offs + ($x as usize)]
			};
		}
		macro_rules! d3 {
			($x:expr) => {
				buffer[d3_offs + ($x as usize)]
			};
		}
...
pub fn inverse_mdct_naive(cached_bd: &CachedBlocksizeDerived, buffer: &mut [f32]) {
	...
	while k2 < n4 {
		...
		v[n2 + k2] = (u[n2 + k2]
			...
			+ c[k2] * (u[n2 + k2 + 1] + u[n - 2 - k2 + 1]))
			/ 2.0;
		v[n - 2 - k2] = (u[n2 + k2] + u[n - 2 - k2]
			- c[k2 + 1] * (u[n2 + k2] - u[n - 2 - k2])
			- c[k2] * (u[n2 + k2 + 1] + u[n - 2 - k2 + 1]))
			/ 2.0;
		v[n2 + 1 + k2] = (u[n2 + 1 + k2] - u[n - 1 - k2] + c[k2 + 1] * (u[n2 + 1 + k2] + u[n - 1 - k2])
			- c[k2] * (u[n2 + k2] - u[n - 2 - k2]))
			/ 2.0;
		v[n - 1 - k2] = (-u[n2 + 1 + k2] + u[n - 1 - k2] + c[k2 + 1] * (u[n2 + 1 + k2] + u[n - 1 - k2])
			- c[k2] * (u[n2 + k2] - u[n - 2 - k2]))
			/ 2.0;
		k2 += 2;
	}
