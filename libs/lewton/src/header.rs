...
macro_rules! convert_to_usize {
	( $val:expr, $val_type:ident ) => {{
		...
		if $val != converted as $val_type {
			try_from!(Err(HeaderReadError::BufferNotAddressable));
		}
		converted
	}};
}
...
( $rdr:expr ) => { {
	...
	let is_vorbis =
		try_from!($rdr.read_u8()) == 0x76 && // 'v'
		try_from!($rdr.read_u8()) == 0x6f && // 'o'
		try_from!($rdr.read_u8()) == 0x72 && // 'r'
		try_from!($rdr.read_u8()) == 0x62 && // 'b'
		try_from!($rdr.read_u8()) == 0x69 && // 'i'
		try_from!($rdr.read_u8()) == 0x73;   // 's'
	if !is_vorbis {
...
pub fn read_header_ident(packet: &[u8]) -> Result<IdentHeader, HeaderReadError> {
	...
	let hd_id = try_from!(read_header_begin(&mut rdr));
	if hd_id != 1 {
		try_from!(Err(HeaderReadError::HeaderBadType(hd_id)));
	}
	let vorbis_version = try_from!(rdr.read_u32());
	if vorbis_version != 0 {
		try_from!(Err(HeaderReadError::UnsupportedVorbisVersion));
	}
	let audio_channels = try_from!(rdr.read_u8());
	let audio_sample_rate = try_from!(rdr.read_u32());
	let bitrate_maximum = try_from!(rdr.read_i32());
	let bitrate_nominal = try_from!(rdr.read_i32());
	let bitrate_minimum = try_from!(rdr.read_i32());
	let blocksize_0 = try_from!(rdr.read_u4());
	let blocksize_1 = try_from!(rdr.read_u4());
...
impl ResidueBook {
	pub fn get_val(&self, i: u8) -> Option<u8> {
		if i >= 8 {
			...
			panic!("Tried to get ResidueBook value out of bounds (index = {})", i);
		}
	...
	fn read_book(rdr: &mut BitpackCursor, vals_used: u8, codebooks: &[Codebook]) -> Result<Self, HeaderReadError> {
		let mut val_i: [u8; 8] = [0; 8];
		for i in 0..7 {
			if vals_used & (1 << i) == 0 {
				continue;
			}
			let val_entry = try_from!(rdr.read_u8());
...
fn lookup_vec_val_decode(lup: &CodebookVqLookup, codebook_entries: u32, codebook_dimensions: u16) -> Vec<f32> {
	let mut value_vectors = Vec::with_capacity(codebook_entries as usize * codebook_dimensions as usize);
	if lup.codebook_lookup_type == 1 {
		...
		for lookup_offset in 0..codebook_entries {
			...
			for _ in 0..codebook_dimensions {
				let multiplicand_offset = (lookup_offset / index_divisor as u32) as usize % codebook_lookup_values;
				let vec_elem = lup.codebook_multiplicands[multiplicand_offset] as f32 * lup.codebook_delta_value
					+ lup.codebook_minimum_value
					+ last;
...
#[rustfmt::skip]
const MAX_BASES_WITHOUT_OVERFLOW: &[u32] = &[
...
#[rustfmt::skip]
const MAX_BASE_MAX_BITS_WITHOUT_OVERFLOW: &[u8] = &[
	0x1f, 0x1f, 0x0f, 0x0a,
	0x07, 0x06, 0x05, 0x04,
	0x03, 0x03, 0x03, 0x02,
	0x02, 0x02, 0x02, 0x02,
	...
	for i in 0..8 {
		...
		if let Some(newselfmul) = u32::checked_mul(selfmul, selfmul) {
			...
			if i < 7 && (exponent >> (i + 1)) > 0 {
				panic!(
					"Overflow when squaring for exp_fast, \
...
fn lookup1_values(codebook_entries: u32, codebook_dimensions: u16) -> u32 {
	...
	for i in 0..max_base_bits + 1 {
		...
		if max_base < base_bits || exp_fast(base_bits, codebook_dimensions as u8) > codebook_entries {
			base_bits &= !cur_disputed_bit;
		}
	}
	return base_bits;
...
fn read_codebook(rdr: &mut BitpackCursor) -> Result<Codebook, HeaderReadError> {
	...
	let sync_pattern = try_from!(rdr.read_u24());
	if sync_pattern != 0x564342 {
		try_from!(Err(HeaderReadError::HeaderBadFormat));
	}

	// 2. Read the _dimension, _entries fields and the ordered bitflag
	let codebook_dimensions = try_from!(rdr.read_u16());
	let codebook_entries = try_from!(rdr.read_u24());
	let ordered = try_from!(rdr.read_bit_flag());

	// 3. Read the codeword lengths
	let mut codebook_codeword_lengths = Vec::with_capacity(convert_to_usize!(codebook_entries, u32));
	if !ordered {
		let sparse = try_from!(rdr.read_bit_flag());
		for _ in 0..codebook_entries {
			let length = if sparse {
				let flag = try_from!(rdr.read_bit_flag());
				if flag {
					try_from!(rdr.read_u5()) + 1
				} else {
		...
		let mut current_length = try_from!(rdr.read_u5()) + 1;
		while current_entry < codebook_entries {
			let number = try_from!(rdr.read_dyn_u32(crate::ilog((codebook_entries - current_entry) as u64)));
	...
	let codebook_lookup_type = try_from!(rdr.read_u4());
	...
	let codebook_lookup: Option<CodebookVqLookup> = if codebook_lookup_type == 0 {
		...
		let codebook_minimum_value = try_from!(rdr.read_f32());
		let codebook_delta_value = try_from!(rdr.read_f32());
		let codebook_value_bits = try_from!(rdr.read_u4()) + 1;
		let codebook_sequence_p = try_from!(rdr.read_bit_flag());
		...
		let mut codebook_multiplicands = Vec::with_capacity(convert_to_usize!(codebook_lookup_values, u64));
		for _ in 0..codebook_lookup_values {
			codebook_multiplicands.push(try_from!(rdr.read_dyn_u32(codebook_value_bits)));
		}
	...
	let codebook_vq_lookup_vec = codebook_lookup
		.as_ref()
		.map(|lup| lookup_vec_val_decode(lup, codebook_entries, codebook_dimensions));

	return Ok(Codebook {
		codebook_dimensions,
		codebook_vq_lookup_vec,
		codebook_huffman_tree: try_from!(VorbisHuffmanTree::load_from_array(&codebook_codeword_lengths)),
	});
}

/// Reads a Floor which is part of the setup header packet.
/// The `codebook_cnt` param is required to check for compliant streams
fn read_floor(rdr: &mut BitpackCursor, codebook_cnt: u16, blocksizes: (u8, u8)) -> Result<Floor, HeaderReadError> {
	let floor_type = try_from!(rdr.read_u16());
	match floor_type {
		0 => {
			let floor0_order = try_from!(rdr.read_u8());
			let floor0_rate = try_from!(rdr.read_u16());
			let floor0_bark_map_size = try_from!(rdr.read_u16());
			let floor0_amplitude_bits = try_from!(rdr.read_u6());
			...
			let floor0_amplitude_offset = try_from!(rdr.read_u8());
			let floor0_number_of_books = try_from!(rdr.read_u4()) + 1;
			let mut floor0_book_list = Vec::with_capacity(convert_to_usize!(floor0_number_of_books, u8));
			for _ in 0..floor0_number_of_books {
				let value = try_from!(rdr.read_u8());
		...
		1 => {
			let floor1_partitions = try_from!(rdr.read_u5());
			let mut maximum_class: i8 = -1;
			let mut floor1_partition_class_list = Vec::with_capacity(floor1_partitions as usize);
			for _ in 0..floor1_partitions {
				let cur_class = try_from!(rdr.read_u4());
			...
			for _ in 0..maximum_class + 1 {
				floor1_class_dimensions.push(try_from!(rdr.read_u3()) + 1);
				let cur_subclass = try_from!(rdr.read_u2());
				floor1_class_subclasses.push(cur_subclass);
				if cur_subclass != 0 {
					let cur_masterbook = try_from!(rdr.read_u8());
				...
				for _ in 0..cur_books_cnt {
					...
					let cur_book = (try_from!(rdr.read_u8()) as i16) - 1;
			...
			let floor1_multiplier = try_from!(rdr.read_u2()) + 1;
			let rangebits = try_from!(rdr.read_u4());
			...
			for cur_class_num in &floor1_partition_class_list {
				for _ in 0..floor1_class_dimensions[*cur_class_num as usize] {
					floor1_x_list.push(try_from!(rdr.read_dyn_u32(rangebits)));
				}
			}
...
/// The `codebook_cnt` param is required to check for compliant streams
fn read_residue(rdr: &mut BitpackCursor, codebooks: &[Codebook]) -> Result<Residue, HeaderReadError> {
	let residue_type = try_from!(rdr.read_u16());
	if residue_type > 2 {
		// Undecodable by spec
		try_from!(Err(HeaderReadError::HeaderBadFormat));
	}
	let residue_begin = try_from!(rdr.read_u24());
	let residue_end = try_from!(rdr.read_u24());
	...
	let residue_partition_size = try_from!(rdr.read_u24()) + 1;
	let residue_classifications = try_from!(rdr.read_u6()) + 1;
	let residue_classbook = try_from!(rdr.read_u8());
	...
	for _ in 0..residue_classifications {
		let mut high_bits = 0;
		let low_bits = try_from!(rdr.read_u3());
		let bitflag = try_from!(rdr.read_bit_flag());
		if bitflag {
			high_bits = try_from!(rdr.read_u5());
		}
	...
	for cascade_entry in &residue_cascade {
		residue_books.push(try_from!(ResidueBook::read_book(rdr, *cascade_entry, codebooks)));
	}
...
fn read_mapping(
	...
	let mapping_type = try_from!(rdr.read_u16());
	if mapping_type > 0 {
		// Undecodable per spec
		try_from!(Err(HeaderReadError::HeaderBadFormat));
	}
	let mapping_submaps = match try_from!(rdr.read_bit_flag()) {
		true => try_from!(rdr.read_u4()) + 1,
		false => 1,
	};
	let mapping_coupling_steps = match try_from!(rdr.read_bit_flag()) {
		true => try_from!(rdr.read_u8()) as u16 + 1,
		false => 0,
	};
	...
	for _ in 0..mapping_coupling_steps {
		let cur_mag = try_from!(rdr.read_dyn_u8(audio_chan_ilog));
		let cur_angle = try_from!(rdr.read_dyn_u8(audio_chan_ilog));
		if (cur_angle == cur_mag) || (cur_mag >= audio_channels) || (cur_angle >= audio_channels) {
			// Undecodable per spec
			try_from!(Err(HeaderReadError::HeaderBadFormat));
		}
	...
	let reserved = try_from!(rdr.read_u2());
	if reserved != 0 {
		// Undecodable per spec
		try_from!(Err(HeaderReadError::HeaderBadFormat));
	}
	...
	for _ in 0..mapping_submaps {
		...
		try_from!(rdr.read_u8());
		let cur_floor = try_from!(rdr.read_u8());
		let cur_residue = try_from!(rdr.read_u8());
		if cur_floor >= floor_count || cur_residue >= residue_count {
			// Undecodable per spec
			try_from!(Err(HeaderReadError::HeaderBadFormat));
		}
...
fn read_mode_info(rdr: &mut BitpackCursor, mapping_count: u8) -> Result<ModeInfo, HeaderReadError> {
	let mode_blockflag = try_from!(rdr.read_bit_flag());
	let mode_windowtype = try_from!(rdr.read_u16());
	let mode_transformtype = try_from!(rdr.read_u16());
	let mode_mapping = try_from!(rdr.read_u8());
	// Verifying ranges
	if mode_windowtype != 0 || mode_transformtype != 0 || mode_mapping >= mapping_count {
		// Undecodable per spec
		try_from!(Err(HeaderReadError::HeaderBadFormat));
	}
	return Ok(ModeInfo {
