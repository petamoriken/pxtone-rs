	...
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		fn fmt_rec(s :&HuffTree, f: &mut fmt::Formatter, depth :u32) -> fmt::Result {
			...
			if s.l.is_some() || s.r.is_some() {
				try_from!(writeln!(f, "ec: {:?}, pl: {:?}, LIS {:?} RIS {:?}",
					s.even_childs, s.payload, s.l.is_some(), s.r.is_some()));
			} else {
				try_from!(writeln!(f, "ec: {:?}, pl: {:?}", s.even_childs, s.payload));
			}
			if let Some(ref v) = s.l {
				depth_print!(f, depth);
				try_from!(write!(f, "LEFT "));
				try_from!(fmt_rec(&*v, f, depth + 1));
			}
			if let Some(ref v) = s.r {
				depth_print!(f, depth);
				try_from!(write!(f, "RIGT "));
				try_from!(fmt_rec(&*v, f, depth + 1));
			}
			return Ok(());
		}
		try_from!(fmt_rec(self, f, 1));
		return Ok(());
	}
} // */
			...
			if !left.even_childs {
				if left.insert_rec(payload, depth - 1) {
					self.even_childs = left.even_childs
						&& if let &mut Some(ref mut right) = &mut self.r.as_mut() {
							right.even_childs
						} else {
							false
						};
...
impl VorbisHuffmanTree {
	...
	pub fn load_from_array(codebook_codeword_lengths: &[u8]) -> Result<VorbisHuffmanTree, HuffmanError> {
		...
		for (i, &codeword_length) in codebook_codeword_lengths.iter().enumerate() {
			...
			if !simple_tree.insert_rec(i as u32, codeword_length) {
				try_from!(Err(HuffmanError::Overspecified)) /* Overspecified, can't be put into tree */
			}
		}
		//println!("The tree:\n{:?}", simple_tree);
		...
		if cnt == 1 {
			...
			if encoded_len == 1 {
				...
				return Ok(VorbisHuffmanTree {
					...
					unrolled_entries: [UnrolledLookupEntry::HasEntry(1, decoded as u32); 256],
				});
			} else {
				// Single entry codebooks must have 1 as their only length entry
				try_from!(Err(HuffmanError::InvalidSingleEntry))
			}
		}

		if !simple_tree.even_childs {
			try_from!(Err(HuffmanError::Underpopulated)); /* Underpopulated */
		}
		...
		fn traverse(tree: &HuffTree, desc_prog: &mut Vec<u32>) -> u32 {
			...
			if has_children {
				desc_prog.push(0);
				desc_prog.push(0);
				desc_prog[cur_pos as usize + 1] = traverse(tree.l.as_ref().unwrap(), desc_prog);
				/*println!("left child of node {:?}: at {:?}", cur_pos,
				desc_prog[cur_pos as usize + 1]);// */
				desc_prog[cur_pos as usize + 2] = traverse(tree.r.as_ref().unwrap(), desc_prog);
		...
		fn uroll_traverse(
			...
			if has_children {
				...
				if prefix_idx == 8 {
					...
					unrolled_entries[prefix as usize] = UnrolledLookupEntry::InconclusiveWithHint(desc_prog_idx);
				} else {
					// Recurse deeper.
					uroll_traverse(
						tree.l.as_ref().unwrap(),
						unrolled_entries,
						prefix + (0 << prefix_idx),
						prefix_idx + 1,
						desc_prog,
						desc_prog[desc_prog_idx as usize + 1],
					);
					uroll_traverse(
						tree.r.as_ref().unwrap(),
						unrolled_entries,
						prefix + (1 << prefix_idx),
						prefix_idx + 1,
						desc_prog,
						desc_prog[desc_prog_idx as usize + 2],
					);
				}
			} else {
				...
				for _ in 1..(1u16 << (8 - prefix_idx)) {
					unrolled_entries[i] = UnrolledLookupEntry::HasEntry(prefix_idx, payload);
					i += it;
				}
			}
		}
		if cnt > 0 {
			uroll_traverse(&simple_tree, &mut unrolled_entries, 0, 0, &desc_prog, 0);
		}
	...
	pub fn iter<'l>(&'l self) -> VorbisHuffmanIter<'l> {
		return VorbisHuffmanIter {
			desc_prog: &self.desc_prog,
			pos: 0,
		};
	}
	...
	pub fn lookup_peeked_data<'l>(&'l self, bit_count: u8, peeked_data: u32) -> PeekedDataLookupResult<'l> {
		if bit_count > 8 {
		...
		return match self.unrolled_entries[peeked_data as usize] {
			...
			// Return in this case.
			HasEntry(cnt_to_remove, payload) if cnt_to_remove <= bit_count => PayloadFound(cnt_to_remove, payload),
			InconclusiveWithHint(hint) => Iter(
				8,
				VorbisHuffmanIter {
					desc_prog: &self.desc_prog,
					pos: hint,
