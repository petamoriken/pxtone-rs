...
use crate::header::{
	IdentHeader, //CommentHeader,
	SetupHeader,
	read_header_ident,
	read_header_setup, //read_header_comment,
};
...
pub unsafe extern "C" fn lewton_context_from_extradata(data: *const u8, len: usize) -> *mut LewtonContext {
	...
	let extradata = unsafe { from_raw_parts(data, len) };
...
#[no_mangle]
pub unsafe extern "C" fn lewton_decode_packet(
	ctx: *mut LewtonContext,
	pkt: *const u8,
	len: usize,
	sample_out: *mut *mut LewtonSamples,
) -> c_int {
	if pkt.is_null() || ctx.is_null() || sample_out.is_null() {
		return 1;
	}
	let (pkt, decoded) = unsafe {
		let pkt = from_raw_parts(pkt, len);
		let decoded = read_audio_packet_generic(&(*ctx).ident_hdr, &(*ctx).setup_hdr, &pkt, &mut (*ctx).pwr);
		(pkt, decoded)
	};
...
#[no_mangle]
pub unsafe extern "C" fn lewton_samples_count(samples: *const LewtonSamples) -> usize {
	unsafe { (*samples).0 }.get(0).map(|v| v.len()).unwrap_or(0)
}

/// Provide a reference to the channel sample data
pub unsafe extern "C" fn lewton_samples_f32(samples: *const LewtonSamples, channel: usize) -> *const f32 {
	unsafe { (*samples).0 }
		.get(channel)
		.map(|v| v.as_ptr())
...
#[no_mangle]
pub unsafe extern "C" fn lewton_samples_drop(samples: *mut LewtonSamples) {
	unsafe {
...
#[no_mangle]
pub unsafe extern "C" fn lewton_context_drop(ctx: *mut LewtonContext) {
	unsafe {
