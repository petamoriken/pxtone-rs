...
fn floor_zero_decode(
	...
	codebooks: &[Codebook],
	fl: &FloorTypeZero,
) -> Result<(Vec<f32>, u64), FloorSpecialCase> {
	// TODO this needs to become 128 bits wide, not just 64,
	// as floor0_amplitude_bits can be up to 127.
	let amplitude = try_from!(rdr.read_dyn_u64(fl.floor0_amplitude_bits));
	if amplitude <= 0 {
	...
	let booknumber = try_from!(rdr.read_dyn_u32(ilog(fl.floor0_number_of_books as u64)));
	match fl.floor0_book_list.get(booknumber as usize) {
		// Undecodable per spec
		None => try_from!(Err(FloorSpecialCase::PacketUndecodable)),
		Some(codebook_idx) => {
			...
			loop {
				...
				let temp_vector = try_from!(rdr.read_huffman_vq(codebook));
...
fn floor_one_decode(
	...
	codebooks: &[Codebook],
	fl: &FloorTypeOne,
) -> Result<Vec<u32>, FloorSpecialCase> {
	...
	floor1_y.push(try_from!(rdr.read_dyn_u8(b)) as u32);
	floor1_y.push(try_from!(rdr.read_dyn_u8(b)) as u32);
	...
	for class in &fl.floor1_partition_class {
		...
		if cbits > 0 {
			...
			cval = try_from!(rdr.read_huffman(&codebooks[cbook].codebook_huffman_tree));
		}
		for _ in 0..cdim {
			...
			if book >= 0 {
				...
				floor1_y.push(try_from!(rdr.read_huffman(tree)));
			} else {
				floor1_y.push(0);
			}
		}
	...
	F: Fn(u32, u32) -> core::cmp::Ordering,
{
	...
	let min_idx = prefix.iter().position(|&val| smaller(val, bound)).unwrap_or_else(|| {
		panic!(
			"No index y < {0} found where v[y] is {1} than v[{0}] = 0x{2:08x}!",
			max_idx, relation, bound
		)
	});

	// Now search for "bigger" entries
	let (offset, max_neighbor) = prefix[min_idx..]
		.iter()
		.cloned()
		.enumerate()
...
fn floor_one_curve_compute_amplitude(floor1_y: &[u32], fl: &FloorTypeOne) -> (Vec<u32>, Vec<bool>) {
	...
	for (i, el) in fl.floor1_x_list.iter().enumerate().skip(2) {
		...
		let predicted = render_point(
			cur_low_neighbor.1,
			floor1_final_y[cur_low_neighbor.0],
			cur_high_neighbor.1,
			floor1_final_y[cur_high_neighbor.0],
			*el,
		) as i32;
		...
		if val > 0 {
			...
			floor1_final_y.push(if val >= room {
				...
				predicted + (if val % 2 == 1 { -val - 1 } else { val } >> 1)
			} as u32);
		} else {
...
#[rustfmt::skip]
const FLOOR1_INVERSE_DB_TABLE: &[f32] = &[
...
fn floor_one_curve_synthesis(
	floor1_final_y: Vec<u32>,
	floor1_step2_flag: Vec<bool>,
	fl: &FloorTypeOne,
	n: u16,
) -> Vec<f32> {
	...
	let floor1_step2_flag_s = |i: usize| floor1_step2_flag[fl.floor1_x_list_sorted[i].0];
	let mut hx = 0;
	let mut lx = 0;
	let mut hy = 0;
...
fn floor_decode<'a>(
	...
	for i in 0..ident.audio_channels as usize {
		...
		let floor_res = match floor {
			&Floor::TypeZero(ref fl) => match floor_zero_decode(rdr, codebooks, fl) {
				Ok((coeff, amp)) => DecodedFloor::TypeZero(coeff, amp, fl),
				Err(Unused) => DecodedFloor::Unused,
				Err(PacketUndecodable) => try_from!(Err(())),
			},
			&Floor::TypeOne(ref fl) => match floor_one_decode(rdr, codebooks, fl) {
...
fn residue_packet_read_partition(
	...
	if resid.residue_type == 0 {
		...
		for i in 0..step {
			let entry_temp = try_from!(rdr.read_huffman_vq(codebook));
		...
		while i < partition_size {
			let entries = try_from!(rdr.read_huffman_vq(codebook));
...
fn residue_packet_decode_inner(
	rdr: &mut BitpackCursor,
	cur_blocksize: u16,
	do_not_decode_flag: &[bool],
	resid: &Residue,
	codebooks: &[Codebook],
) -> Result<Vec<f32>, ()> {
		...
		macro_rules! eno {
			($expr:expr) => {
				match $expr {
					...
					::core::result::Result::Err(_) => break 'pseudo_return,
				}
			};
		}
				...
				for _ in 0..classwords_per_codeword {
					...
					for (j, do_not_decode) in do_not_decode_flag.iter().enumerate() {
						...
						if let Some(vqbook) = vqbook_opt {
							...
							match residue_packet_read_partition(rdr, codebook, resid, vec_j_offs) {
								Ok(_) => (),
								Err(err) => {
									use crate::header::HuffmanVqReadErr::*;
									match err {
										EndOfPacket => break 'pseudo_return,
										NoVqLookupForCodebook => panic!("Codebook must have a value mapping"),
									}
								},
							}
						}
...
fn residue_packet_decode(
	...
	if resid.residue_type == 2 {
		...
		if !to_decode_found {
			...
			let vectors = try_from!(residue_packet_decode_inner(
				rdr,
				cur_blocksize * ch as u16,
				&c_do_not_decode_flag,
				resid,
				codebooks
			));
		...
		return residue_packet_decode_inner(rdr, cur_blocksize, do_not_decode_flag, resid, codebooks);
	}
}
...
fn dual_mut_idx<T>(v: &mut [T], idx_a: usize, idx_b: usize) -> (&mut T, &mut T) {
	assert_ne!(idx_a, idx_b, "not allowed, indices must be different!");

	let range = if idx_a < idx_b {
		idx_a..idx_b + 1
	} else {
		idx_b..idx_a + 1
	};
...
fn test_imdct_slow() {
	...
	if mismatches > mismatches_limit {
		panic!(
			"Numer of mismatches {} was larger than limit of {}",
...
pub fn get_decoded_sample_count(
	...
	if try_from!(rdr.read_bit_flag()) {
		try_from!(Err(AudioReadError::AudioIsHeader));
	}
	let mode_number = try_from!(rdr.read_dyn_u8(ilog(setup.modes.len() as u64 - 1)));
	let mode = &setup.modes[mode_number as usize];
	let bs = if mode.mode_blockflag {
		ident.blocksize_1
	} else {
		ident.blocksize_0
	};
	...
	let previous_next_window_flag = if mode.mode_blockflag {
		Some((try_from!(rdr.read_bit_flag()), try_from!(rdr.read_bit_flag())))
	} else {
		None
	};
...
*/
pub fn read_audio_packet_generic<S: Samples>(
	ident: &IdentHeader,
	setup: &SetupHeader,
	packet: &[u8],
	...
	let mode = if let Some(mode) = setup.modes.get(mode_number as usize) {
		...
		try_from!(Err(AudioReadError::AudioBadFormat))
	};
	...
	// Decode the floors
	let decoded_floor_infos = try_from!(floor_decode(&mut rdr, ident, mapping, &setup.codebooks, &setup.floors));
	...
	// and also propagate
	for (&mag, &angle) in mapping.mapping_magnitudes.iter().zip(mapping.mapping_angles.iter()) {
	...
	for (i, &residue_number) in mapping.mapping_submap_residues.iter().enumerate() {
		...
		let vectors = match residue_packet_decode(&mut rdr, n, &do_not_decode_flag, cur_residue, &setup.codebooks) {
			Ok(v) => v,
	...
	// Inverse coupling
	for (&mag, &angle) in mapping
		.mapping_magnitudes
		.iter()
		.rev()
		.zip(mapping.mapping_angles.iter().rev())
	{
		let (mag_vector, angle_vector) = dual_mut_idx(&mut residue_vectors, mag as usize, angle as usize);
	...
	for (residue_vector, chan_decoded_floor) in residue_vectors.iter().zip(decoded_floor_infos.iter()) {
		let mut floor_decoded: Vec<f32> = match chan_decoded_floor {
			&DecodedFloor::TypeZero(ref coefficients, amplitude, ref fl) => {
				floor_zero_compute_curve(coefficients, amplitude, fl, mode.mode_blockflag, n / 2)
			},
			&DecodedFloor::TypeOne(ref floor_y, ref fl) => {
				let (floor1_final_y, floor1_step2_flag) = floor_one_curve_compute_amplitude(floor_y, fl);
				floor_one_curve_synthesis(floor1_final_y, floor1_step2_flag, fl, n / 2)
			},
			&DecodedFloor::Unused => {
	...
	for ref mut spectrum in audio_spectri.iter_mut() {
		...
		//crate::imdct::inverse_mdct_naive(cached_bd, &mut spectrum[..]);
		crate::imdct::inverse_mdct(cached_bd, &mut spectrum[..], bs);
	...
	// Compute windowing info for right window
	let (right_win_start, right_win_end) = if previous_next_window_flag.map_or(true, |(_, next_win_flag)| next_win_flag)
	{
		(window_center, n)
	} else {
		let bs_0_exp = 1 << ident.blocksize_0;
	...
	if let Some(prev_data) = pwr.data.take() {
		...
		for chan in audio_spectri.iter_mut() {
			let mut future_prev_half = Vec::with_capacity((right_win_end - right_win_start) as usize);
