...
use crate::event::{read_var_int, EVENTDEFAULT_BASICKEY};
...
pub enum WoiceType { #[default] None, Pcm, Ptv, Ptn, OggVorbis }

#[derive(Clone, Copy, Debug, PartialEq, Eq, Default)]
pub enum VoiceType { #[default] Coodinate, Overtone, Noise, Sampling, OggVorbis }
...
impl VoiceInstance {
    ...
    pub fn get_sample_i16(&self, frame: usize, ch: usize) -> i16 {
        ...
        if offset + 1 >= self.samples_w.len() { return 0; }
...
impl Woice {
    ...
    pub fn read_mate_pcm<R: Read + Seek>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if voice_flags & VOICE_FLAG_UNCOVERED != 0 { return Err(PxtoneError::UnknownFormat); }

        let sample_num = data_size as i32 * 8 / bps / ch;
        let mut pcm = Pcm::create(ch, sps, bps, sample_num)
            .ok_or(PxtoneError::UnknownFormat)?;
    ...
    pub fn read_mate_ptn<R: Read + Seek>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if rrr > 1 || rrr < 0 { return Err(PxtoneError::UnknownFormat); }
    ...
    pub fn read_mate_oggv<R: Read + Seek>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        unit.basic_key   = basic_key;
        unit.tuning      = tuning;
        unit.ogg_data    = Some(OggData { ch, sps, smp_num, data });
        self.x3x_basic_key = basic_key;
        self.x3x_tuning    = 0.0;
        self.voices = vec![unit];
    ...
    fn ptv_read<R: Read>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if &code != b"PTVOICE-" { return Err(PxtoneError::InvalidCode); }

        let version = r.read_i32::<LE>()?;
        let _total  = r.read_i32::<LE>()?;
        if version > 20060111 { return Err(PxtoneError::NewFormat); }

        let x3x_basic_key = read_var_int(r)?;
        let work1         = read_var_int(r)?;
        let work2         = read_var_int(r)?;
        if work1 != 0 || work2 != 0 { return Err(PxtoneError::UnknownFormat); }
        ...
        for _ in 0..voice_num {
            ...
            if unit.voice_flags & VOICE_FLAG_UNCOVERED != 0 { return Err(PxtoneError::UnknownFormat); }
            if unit.data_flags  & DATA_FLAG_UNCOVERED  != 0 { return Err(PxtoneError::UnknownFormat); }

            if unit.data_flags & DATA_FLAG_WAVE     != 0 { ptv_read_wave(r, &mut unit)?; }
            if unit.data_flags & DATA_FLAG_ENVELOPE != 0 { ptv_read_envelope(r, &mut unit)?; }
                ...
                VoiceType::Sampling => {
                    if let Some(pcm) = &unit.pcm {
                        let mut work = Pcm::create(pcm.ch, pcm.sps, pcm.bps, pcm.smp_body)
                            .ok_or(PxtoneError::PcmConvert)?;
                        let src = pcm.samples();
                        let copy_len = src.len().min(work.samples().len());
                        work.samples_mut()[..copy_len]
    ...
    pub fn tone_ready_envelope(&mut self, sps: i32) -> Result<(), PxtoneError> {
        for (unit, inst) in self.voices.iter().zip(self.instances.iter_mut()) {
            ...
            if env.head_num > 0 {
                let mut size = 0i32;
                for e in 0..env.head_num as usize { size += env.points[e].0; }
                inst.env_size = (size as f64 * sps as f64 / env.fps as f64) as i32;
                if inst.env_size == 0 { inst.env_size = 1; }
...
fn ptv_read_envelope<R: Read>(r: &mut R, unit: &mut VoiceUnit) -> Result<(), PxtoneError> {
    ...
    if body_num != 0      { return Err(PxtoneError::UnknownFormat); }
    if tail_num != 1      { return Err(PxtoneError::UnknownFormat); }
...
fn update_wave_ptv(unit: &VoiceUnit, inst: &mut VoiceInstance, ch: i32, _sps: i32, bps: i32) {
    ...
    let pan_vol: [i32; 2] = if ch == 2 {
        if unit.pan > 64 { [128 - unit.pan, 64] }
        else if unit.pan < 64 { [64, unit.pan] }
        else { [64, 64] }
    } else { [64, 64] };

    let pts: Vec<Point> = unit.wave.points.iter().map(|&(x, y)| Point { x, y }).collect();
    ...
    if bps == 8 {
        for s in 0..smp_body {
            let osc = if b_ovt { osci.get_one_sample_overtone(s as i32) }
                      else     { osci.get_one_sample_coodinate(s as i32) };
            for c in 0..ch as usize {
                let mut work = osc * pan_vol[c] as f64 / 64.0;
                if work > 1.0  { work = 1.0;  inst.b_sine_over = true; }
                if work < -1.0 { work = -1.0; inst.b_sine_over = true; }
