...
impl Delay {
    ...
    pub fn tone_ready(&mut self, beat_num: i32, beat_tempo: f32, sps: i32) {
        ...
        if self.freq == 0.0 || self.rate == 0.0 { return; }
        ...
        self.smp_num = match self.unit {
            ...
            DelayUnit::Meas   => (sps as f64 * 60.0 * beat_num as f64 / beat_tempo as f64 / self.freq as f64) as usize,
    ...
    pub fn tone_supple(&mut self, ch: usize, group_smps: &mut [i32]) {
        if self.smp_num == 0 { return; }
        let a = self.bufs[ch][self.offset] * self.rate_s32 / 100;
        if self.played { group_smps[self.group] += a; }
