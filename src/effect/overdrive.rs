...
impl OverDrive {
    ...
    pub fn read<R: Read + Seek>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if xxx != 0 { return Err(PxtoneError::UnknownFormat); }
        if yyy != 0.0 { return Err(PxtoneError::UnknownFormat); }
        if cut < CUT_MIN || cut > CUT_MAX { return Err(PxtoneError::UnknownFormat); }
        if amp < AMP_MIN || amp > AMP_MAX { return Err(PxtoneError::UnknownFormat); }
