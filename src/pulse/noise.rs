...
impl Noise {
    ...
    /// 各パラメータを有効範囲にクランプ
    pub fn fix(&mut self) {
        if self.smp_num_44k > LIMIT_SMP_NUM { self.smp_num_44k = LIMIT_SMP_NUM; }
    ...
    pub fn read<R: Read>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if &code != CODE { return Err(PxtoneError::InvalidCode); }

        let mut ver_buf = [0u8; 4];
        r.read_exact(&mut ver_buf)?;
        let ver = u32::from_le_bytes(ver_buf);
        if ver > VER { return Err(PxtoneError::NewFormat); }
        ...
        if unit_num < 0                          { return Err(PxtoneError::InvalidData); }
        if unit_num as usize > MAX_UNIT_NUM      { return Err(PxtoneError::UnknownFormat); }

        self.units.clear();
        for _ in 0..unit_num {
            let mut unit = NoiseUnit { enabled: true, ..Default::default() };

            let flags = read_var_int(r)? as u32;
            if flags & FLAG_UNCOVERED != 0 { return Err(PxtoneError::UnknownFormat); }

            if flags & FLAG_ENVELOPE != 0 {
                let enve_num = read_var_int(r)?;
                if enve_num as usize > MAX_ENVELOPE_NUM { return Err(PxtoneError::UnknownFormat); }
                for _ in 0..enve_num {
            ...
            if flags & FLAG_OSC_MAIN != 0 { read_oscillator(r, &mut unit.main)?; }
            if flags & FLAG_OSC_FREQ != 0 { read_oscillator(r, &mut unit.freq)?; }
            if flags & FLAG_OSC_VOLU != 0 { read_oscillator(r, &mut unit.volu)?; }
...
fn fix_osc(osc: &mut NoiseOscillator) {
    if osc.freq   >  LIMIT_OSC_FREQUENCY { osc.freq   = LIMIT_OSC_FREQUENCY; }
    if osc.freq   <= 0.0                 { osc.freq   = 0.0; }
    if osc.volume >  LIMIT_OSC_VOLUME    { osc.volume = LIMIT_OSC_VOLUME; }
    if osc.volume <= 0.0                 { osc.volume = 0.0; }
    if osc.offset >  LIMIT_OSC_OFFSET    { osc.offset = LIMIT_OSC_OFFSET; }
    if osc.offset <= 0.0                 { osc.offset = 0.0; }
}
