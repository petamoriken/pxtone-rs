...
impl Pcm {
    ...
    pub fn samples(&self) -> &[u8] { &self.samples }
    pub fn samples_mut(&mut self) -> &mut [u8] { &mut self.samples }
    ...
    pub fn read_wav<R: Read + Seek>(r: &mut R) -> Result<Self, PxtoneError> {
        ...
        if format_id != 0x0001              { return Err(PxtoneError::UnknownFormat); }
        if ch != 1 && ch != 2              { return Err(PxtoneError::UnknownFormat); }
        if bps != 8 && bps != 16           { return Err(PxtoneError::UnknownFormat); }
        ...
        let data_size = loop {
            ...
            if &tag == b"data" { break chunk_size; }
                ...
                match self.bps {
                    ...
                    16 => {
                        for i in 0..total {
                            ...
                            w[i * 4    ] = s[0]; w[i * 4 + 1] = s[1];
                            w[i * 4 + 2] = s[0]; w[i * 4 + 3] = s[1];
                        }
                    }
    ...
    fn convert_sps(&mut self, new_sps: i32) -> Result<(), PxtoneError> {
        if self.sps == new_sps { return Ok(()); }
        let bytes_per_sample = (self.ch * self.bps / 8) as usize;
        let old_total = self.total_samples() as usize;
        let new_head = ((self.smp_head as f64 * new_sps as f64 + self.sps as f64 - 1.0) / self.sps as f64) as usize;
        let new_body = ((self.smp_body as f64 * new_sps as f64 + self.sps as f64 - 1.0) / self.sps as f64) as usize;
        let new_tail = ((self.smp_tail as f64 * new_sps as f64 + self.sps as f64 - 1.0) / self.sps as f64) as usize;
