use crate::event::{
    EVENTDEFAULT_GROUPNO, EVENTDEFAULT_VELOCITY, EVENTDEFAULT_VOLUME,
    EVENTDEFAULT_TUNING, EVENTDEFAULT_KEY,
};
use crate::woice::{VoiceInstance, BUFSIZE_TIMEPAN, VOICE_FLAG_SMOOTH, VOICE_FLAG_WAVELOOP};
...
impl Unit {
    ...
    pub fn tone_clear(&mut self) {
        for ch in 0..MAX_CHANNEL {
            for v in &mut self.pan_time_bufs[ch] { *v = 0; }
        }
    }
    ...
    pub fn tone_zero_lives(&mut self) {
        for i in 0..MAX_UNIT_CONTROL_VOICE { self.tones[i].life_count = 0; }
    }
    ...
    pub fn tone_pan_volume(&mut self, ch: i32, pan: i32) {
        ...
        self.pan_vols[1] = 64;
        if ch == 2 {
            if pan >= 64 { self.pan_vols[0] = 128 - pan; }
            else         { self.pan_vols[1] = pan; }
        }
    }
    ...
    pub fn tone_velocity (&mut self, val: i32) { self.v_velocity           = val; }
    pub fn tone_volume   (&mut self, val: i32) { self.v_volume             = val; }
    pub fn tone_portament(&mut self, val: i32) { self.portament_sample_num = val; }
    pub fn tone_groupno  (&mut self, val: i32) { self.v_groupno            = val; }
    pub fn tone_tuning   (&mut self, val: f32) { self.v_tuning             = val; }
            ...
            if vt.life_count > 0 && vi.env_size > 0 {
                if vt.on_count > 0 {
                    ...
                    vt.env_volume = vt.env_start
                        + (0 - vt.env_start) * vt.env_pos / vi.env_release.max(1);
                    vt.env_pos += 1;
                }
            }
        }
        ...
        for ch in 0..MAX_CHANNEL {
            ...
            for v in 0..self.voice_num {
                ...
                if vt.life_count > 0 {
                    ...
                    if vi.env_size > 0 { work = work * vt.env_volume / 128; }
    ...
    pub fn tone_supple(&self, group_smps: &mut [i32], ch: usize, time_pan_index: usize) {
        let idx = (time_pan_index + BUFSIZE_TIMEPAN - self.pan_times[ch] as usize) & (BUFSIZE_TIMEPAN - 1);
    ...
    pub fn tone_increment_sample(&mut self, freq: f32, instances: &[VoiceInstance]) {
        for v in 0..self.voice_num {
            let vi  = &instances[v];
            let vt  = &mut self.tones[v];
            if vt.life_count > 0 { vt.life_count -= 1; }
            if vt.life_count > 0 {
                if vt.on_count > 0 { vt.on_count -= 1; }
                ...
                if vt.smp_pos >= body {
                    if self.voice_flags.get(v).copied().unwrap_or(0) & VOICE_FLAG_WAVELOOP != 0 {
                        vt.smp_pos -= body;
                        if vt.smp_pos >= body { vt.smp_pos = 0.0; }
                    } else {
                        vt.life_count = 0;
                    }
                }
