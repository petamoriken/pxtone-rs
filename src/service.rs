...
use crate::event::{
    ...
    EVENTDEFAULT_VOICENO, EVENTDEFAULT_BASICKEY,
    EVENTKIND_ON, EVENTKIND_KEY, EVENTKIND_PAN_VOLUME, EVENTKIND_PAN_TIME,
    EVENTKIND_VELOCITY, EVENTKIND_VOLUME, EVENTKIND_PORTAMENT,
    EVENTKIND_VOICENO, EVENTKIND_GROUPNO, EVENTKIND_TUNING,
};
use crate::master::Master;
use crate::text::Text;
use crate::woice::{Woice, VoiceInstance, VOICE_FLAG_BEATFIT, BUFSIZE_TIMEPAN};
use crate::unit::Unit;
...
enum FmtVer { X1x, X2x, X3x, X4x, V5 }
...
impl VomitPreparation {
    pub fn new() -> Self {
        Self { master_volume: 1.0, ..Default::default() }
    }
}
...
impl PxtoneService {
    ...
    fn read_version<R: Read>(&self, r: &mut R) -> Result<FmtVer, PxtoneError> {
        ...
        if &ver == CODE_PROJ_X1X { return Ok(FmtVer::X1x); }
        if &ver == CODE_PROJ_X2X { return Ok(FmtVer::X2x); }
        if &ver == CODE_TUNE_X2X { return Ok(FmtVer::X2x); }

        let fmt_ver = if      &ver == CODE_PROJ_X3X || &ver == CODE_TUNE_X3X { FmtVer::X3x }
                      else if &ver == CODE_PROJ_X4X || &ver == CODE_TUNE_X4X { FmtVer::X4x }
                      else if &ver == CODE_PROJ_V5  || &ver == CODE_TUNE_V5  { FmtVer::V5  }
                      else { return Err(PxtoneError::UnknownFormat); };
    ...
    fn read_tune_items<R: Read + Seek>(&mut self, r: &mut R, _fmt_ver: FmtVer)
        -> Result<(), PxtoneError>
    {
        loop {
            ...
            match &code {
                ...
                b"num UNIT" => {
                    ...
                    if size != 4 { return Err(PxtoneError::UnknownFormat); }
                    let num = r.read_i16::<LE>()? as usize;
                    let rrr = r.read_i16::<LE>()?;
                    if rrr != 0                  { return Err(PxtoneError::UnknownFormat); }
                    if num > MAX_UNIT_NUM        { return Err(PxtoneError::UnknownFormat); }
                ...
                b"matePCM " | b"matePCM=" => {
                    if self.woices.len() >= MAX_WOICE_NUM { return Err(PxtoneError::WoiceFull); }
                    let mut w = Woice::new(); w.read_mate_pcm(r)?; self.woices.push(w);
                }
                b"matePTV " => {
                    if self.woices.len() >= MAX_WOICE_NUM { return Err(PxtoneError::WoiceFull); }
                    let mut w = Woice::new(); w.read_mate_ptv(r)?; self.woices.push(w);
                }
                b"matePTN " => {
                    if self.woices.len() >= MAX_WOICE_NUM { return Err(PxtoneError::WoiceFull); }
                    let mut w = Woice::new(); w.read_mate_ptn(r)?; self.woices.push(w);
                }
                b"mateOGGV" => {
                    if self.woices.len() >= MAX_WOICE_NUM { return Err(PxtoneError::WoiceFull); }
                    let mut w = Woice::new(); w.read_mate_oggv(r)?; self.woices.push(w);
                }
                b"effeDELA" => {
                    if self.delays.len() >= MAX_DELAY_NUM { return Err(PxtoneError::UnknownFormat); }
                    let mut d = Delay::new(); d.read(r)?; self.delays.push(d);
                }
                b"effeOVER" => {
                    if self.overdrives.len() >= MAX_OVERDRIVE_NUM { return Err(PxtoneError::UnknownFormat); }
                    let mut od = OverDrive::new(); od.read(r)?; self.overdrives.push(od);
                }
    ...
    fn read_assi_woic<R: Read>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if rrr != 0                           { return Err(PxtoneError::UnknownFormat); }
        if woice_index >= self.woices.len()   { return Err(PxtoneError::UnknownFormat); }
    ...
    fn read_assi_unit<R: Read>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        ...
        if rrr != 0                          { return Err(PxtoneError::UnknownFormat); }
        if unit_index >= self.units.len()    { return Err(PxtoneError::UnknownFormat); }
    ...
    fn read_old_unit_v1<R: Read>(&mut self, r: &mut R) -> Result<(), PxtoneError> {
        if self.units.len() >= MAX_UNIT_NUM { return Err(PxtoneError::UnknownFormat); }
        ...
        self.events.add_i(0, u_idx as u8, EVENTKIND_VOICENO, u_idx as i32);
        Ok(())
    }
    ...
    fn x3x_tuning_key_event(&mut self) -> Result<(), PxtoneError> {
        ...
        for u in 0..unit_num {
            ...
            let has_key = self.events.records().iter()
                .any(|e| e.unit_no == u as u8 && e.kind == EVENTKIND_KEY);
            if !has_key {
                self.events.add_i(0, u as u8, EVENTKIND_KEY, 0x6000);
            }
            self.events.value_change(0, -1, u as u8, EVENTKIND_KEY, change);
        }
        Ok(())
    }
    ...
    fn tones_clear(&mut self) {
        for d in &mut self.delays   { d.tone_clear(); }
        for u in &mut self.units    { u.tone_clear(); }
    }
    ...
    pub fn moo_preparation(&mut self, prep: Option<&VomitPreparation>) -> Result<(), PxtoneError> {
        ...
        if let Some(p) = prep {
            ...
            if p.meas_end    != 0   { meas_end    = p.meas_end;    }
            if p.meas_repeat != 0   { meas_repeat = p.meas_repeat; }
            if p.fadein_sec  != 0.0 { fadein_sec  = p.fadein_sec;  }
        ...
        self.moo_clock_rate = 60.0 * self.dst_sps as f64
            / (self.moo_bt_tempo as f64 * self.moo_bt_clock as f64);
        ...
        if fadein_sec > 0.0 { self.moo_set_fade(1, fadein_sec); }
        else                { self.moo_set_fade(0, 0.0); }
    ...
    fn moo_reset_voice_on(&mut self, unit_idx: usize, woice_idx: usize) {
        if self.woices.is_empty() { return; }
        if unit_idx >= self.units.len() { return; }

        let woice_idx = woice_idx.min(self.woices.len() - 1);

        // ウォイスから voice_flags を収集
        let voice_num  = self.woices[woice_idx].voices.len();
        let voice_flags: Vec<u32> = self.woices[woice_idx].voices
            .iter().map(|v| v.voice_flags).collect();
    ...
    fn moo_pxtone_sample(&mut self, out: &mut [i16; 2]) -> bool {
        ...
        for ch in 0..ch_num {
            for g in 0..group_num { group_smps[g] = 0; }
        ...
        for d in &mut self.delays { d.tone_increment(); }

        // ---- 6. フェード処理 ----
        if self.moo_fade_fade < 0 {
            if self.moo_fade_count > 0 { self.moo_fade_count -= 1; }
            else { return false; }
        ...
        if self.moo_smp_count >= self.moo_smp_end {
            if !self.moo_b_loop { return false; }
        ...
        match ev.kind {
            EVENTKIND_ON => {
                ...
                for v in 0..voice_num {
                    ...
                    let env_release = self.woices.get(wi)
                        .and_then(|w| w.instances.get(v))
                        .map(|i| i.env_release)
                        .unwrap_or(0);
                    let env_size = self.woices.get(wi)
                        .and_then(|w| w.instances.get(v))
                        .map(|i| i.env_size)
                        .unwrap_or(0);

                    // tones の env_release_clock を読み出し（imm borrow of self.units）
                    let tone_rls_clock = self.units[u].tones.get(v)
    ...
    pub fn moo(&mut self, buf: &mut [u8]) -> bool {
        if !self.b_valid_data { return false; }
        if self.b_end_vomit   { return false; }

        let byte_per_smp = (self.dst_ch_num * 2) as usize;
        if buf.len() % byte_per_smp != 0 { return false; }
    ...
    pub fn is_end_vomit   (&self) -> bool  { self.b_end_vomit  }
    pub fn is_valid_data  (&self) -> bool  { self.b_valid_data }
    ...
    pub fn moo_get_sampling_offset(&self) -> i32 {
        if self.b_end_vomit { 0 } else { self.moo_smp_count }
    }

    pub fn moo_get_sampling_end(&self) -> i32 {
        if self.b_end_vomit { 0 } else { self.moo_smp_end }
    }

    pub fn moo_get_total_sample(&self) -> i32 { self.calc_total_sample() }
}
